...
class PoseidonBase {
 protected:
  ...
  std::vector<uint8_t> DoGetState() const {
    ...
    CHECK(buffer.Grow(base::EstimateSize(poseidon_.state)));
    CHECK(buffer.Write(poseidon_.state));
    CHECK(buffer.Done());
  ...
  void DoSetState(absl::Span<const uint8_t> state) {
    ...
    buffer.set_endian(base::Endian::kLittle);
    CHECK(buffer.Read(&poseidon_.state));
    CHECK(buffer.Done());
  }
...
class PoseidonWriter : public crypto::TranscriptWriter<AffinePoint>,
 ...
 public:
  ...
  size_t GetStateLen() const {
    return base::EstimateSize(this->poseidon_.state);
  }
