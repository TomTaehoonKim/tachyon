...
class Radix2EvaluationDomain : public UnivariateEvaluationDomain<F, MaxDegree> {
 ...
 private:
  ...
  static std::vector<F> GenerateTwiddleLookupTable(const F& omega,
    ...
    // dense
    if (is_lut_len_large) {
      F::GetSuccessivePowers(size_t{1} << log_n, omega);
    }
...
#pragma omp parallel
    {
      size_t low_num_elements_per_thread =
          base::GetNumElementsPerThread(low_degree_lut);
#pragma omp for nowait
      for (size_t i = 0; i < low_degree_lut.size();
           i += low_num_elements_per_thread) {
        F w_n = omega.Pow(i);
        for (size_t j = i; j < std::min(i + low_num_elements_per_thread,
                                        low_degree_lut.size());
             ++j) {
          low_degree_lut.at(j) = w_n;
          w_n *= omega;
        }
      }

      size_t high_num_elements_per_thread =
          base::GetNumElementsPerThread(high_degree_lut);
      OPENMP_PARALLEL_FOR(size_t i = 0; i < high_degree_lut.size();
                          i += high_num_elements_per_thread) {
        F w_n = high_degree_omega.Pow(i);
        for (size_t j = i; j < std::min(i + high_num_elements_per_thread,
                                        high_degree_lut.size());
             ++j) {
          high_degree_lut.at(j) = w_n;
  ...
  static void ParallelFFT(PolyOrEvals& a, const F& omega, uint32_t log_n) {
    ...
    // shuffle
    OPENMP_PARALLEL_FOR(size_t i = 0; i < a.size(); ++i) {
      size_t j = i / sub_n;
      size_t k = i % sub_n;
      a.at(i) = tmp.at(k * split_m + j);
    }
